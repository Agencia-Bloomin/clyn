...
class Info
{
    ...
    public function getAllMainCategories()
    {
        ...
        $categories =  (new CategoryModel)->find("father_id = :father_id", "father_id=0")->fetch(true);
        if (!empty($categories)) {
            foreach ($categories as $cat) {
                // if (!empty($cat->url) &&  substr($cat->url, -1) !== "/") {
                //     $cat->url = $cat->url . "/";
    ...
    public function getAllCategoriesByFatherId(int $father_id)
    {
        ...
        $categories =  (new CategoryModel)->find("father_id = :father_id", "father_id=$father_id")->fetch(true);
        if (!empty($categories)) {
            foreach ($categories as $cat) {
                // if (!empty($cat->url) && substr($cat->url, -1) !== "/") {
                //     $cat->url = $cat->url . "/";
